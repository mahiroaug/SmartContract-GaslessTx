...
const TOKEN_ABI =
    require("../artifacts/contracts/MahiroCoin_ERC20SingleShot.sol/MahiroCoinSingleShot.json").abi;
const FORWARDER_CA = process.env.FORWARDER_CA;
const FORWARDER_ABI =
    require("../artifacts/contracts/MahiroCoin_IForwarder.sol/IForwarder.json").abi;
...
async function getAccountBalance(address) {
    ...
    console.log(
        `${coinName} Balance: ${web3.utils.fromWei(
            coinBalance,
            "ether"
        )} ${coinSymbol}`
    );
...
const sendTx = async (_to, _tx, _signer, _gasLimit) => {
    ...
    const gasPriceInGwei = await web3_withRelayer.utils.fromWei(
        gasPrice,
        "gwei"
    );
...
(async () => {
    ...
    console.log(
        `forwarder address: ${FORWARDER_CA}, verifying: ${web3.utils.isAddress(
            FORWARDER_CA
        )}`
    );
    console.log(
        `token address: ${TOKEN_CA}, verifying: ${web3.utils.isAddress(
            TOKEN_CA
        )}`
    );
