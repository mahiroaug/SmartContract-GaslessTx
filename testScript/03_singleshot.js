...
async function sendSingleShot(req,signature,payerAddr){
    try{
        const tx = await token_withRelayer.methods.singleShot(
            req.owner,
            req.spender,
            req.recipient,
            req.value,
...
(async() => {
    ...
    await sendSingleShot(req,signature,relayerAddr);
    // ******* close FIREBLOCKS *****************************
    ...
    console.log(`allowance from owner to relayer is ${allowance} ${req.symbol}`);
